...
MatchResult Detect2Chars::doMatch(const QString &text, int offset, const QStringList &) const
{
...
MatchResult KeywordListRule::doMatch(const QString &text, int offset, const QStringList &) const
{
    ...
    if (m_keywordList.contains(QStringView(text).mid(offset, newOffset - offset), m_caseSensitivity)) {
        return newOffset;
    }
