...
foreach my $name (sort keys %themeToHTML) {
    ...
    print $F $fullFile;
    close $F;
    $themeToHTMLSnippet{$name} = $snippetName;
}

# {{< rawhtml >}}
# {{% include "/static/themes/html/ayu-light.html" %}}
# {{< /rawhtml >}}
...
foreach my $name (sort keys %themeToHTMLSnippet) {
    my $file = $themeToHTMLSnippet{$name};
